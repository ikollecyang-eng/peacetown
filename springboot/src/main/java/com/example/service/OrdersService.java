...
//        orders.setTime(currentTimeStr);
        ...
        for (Cart cart : orders.getCartData()) {
            ...
            dbOrders.setTime(currentTimeStr);
            ...
            int num = goods.getNum() - cart.getNum();
            if(num<0)
                throw new CustomException(ResultCodeEnum.GOOD_NUM_LIMIT);

            goods.setNum(num);
            goods.setSaleCnt(goods.getSaleCnt()+cart.getNum());
