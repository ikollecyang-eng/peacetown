      ...
      <div style="padding-bottom: 10px">
        <div style="display: flex; font-size: 18px; color: #000000FF; line-height: 80px; border-bottom: #cccccc 1px solid;">
          ...
          <div style="flex: 1; font-size: 16px; text-align: right; padding-right: 20px">
            已选商品 ￥ {{ Number(totalPrice.toFixed(4))  }} <el-button type="danger" round @click="pay">下单</el-button>
          </div>
        </div>
...
<!--                <template v-slot="scope">-->
<!--                  <a :href="'/front/business?id=' + scope.row.supplierId">{{scope.row.supplierName}}</a>-->
<!--                </template>-->
<!--              </el-table-column>-->
              <el-table-column prop="goodsPrice" label="商品价格"></el-table-column>
              <el-table-column prop="num" label="选择数量">
                <template v-slot="scope">
                  <el-input-number v-model="scope.row.num" style="width: 100px" @change="handleChange(scope.row)" :min="1" :max=scope.row.goodsNum ></el-input-number>
                </template>
              </el-table-column>
